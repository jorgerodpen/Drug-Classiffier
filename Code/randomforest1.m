...
% 'Gender', 'Education', 'Country', 'Ethnicity'. Personality tests contain
% numerical values. The five factor model ('Neuroticism', 'Extraversion',
% 'Openness to experience', 'Agreeableness', 'Conciestiousness') contain 
% values from  12 to 60, while the 'Impulsiveness' and 'Sensation Seeking'
% tests are scored from 0 to 9 and 10, respectively. 
...
% We used the means of 'minLeafSize' (3) and 'numPredictors' (1) obtained 
...
%     tic
%     basetree = templateTree('MinLeafSize', 3, ... % Size of each leaf  
...
%% Trees vs time plots
figure(2)
subplot(2,1,1), plot(trees, AUCvst, 'LineWidth', 2)
xlabel('# trees')
ylabel('AUC')
grid
ax = gca;
ax.FontSize = 18;
subplot(2,1,2), plot(trees, t, 'LineWidth', 2)
xlabel('# trees')
...
% %==Cross validation
% AUCcv = crossvalidationrf(Xtrain, ytrain, 10, 3, 1, 600);
% 
% %==Bootstrap validation
% AUCb = bootstraprf(Xtrain, ytrain, 3, 1, 600);
...
rf = fitcensemble(Xtrain, ytrain, 'Method', 'Bag', ...
        'Learners', basetree, 'ClassNames', [0; 1], ...
        'NumLearningCycles', 400); % Number of trees 
traintime = toc;
